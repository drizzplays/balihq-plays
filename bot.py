...
def _moneyline_name_display(bet: str) -> str:
    ...
    raw = re.sub(r"\bMONEY\s*LINE\b", "", raw, flags=re.IGNORECASE)
    raw = re.sub(r"\bML\b", "", raw, flags=re.IGNORECASE)
    raw = re.sub(r"[+-]\d{2,4}", "", raw)
    raw = re.sub(r"\s+", " ", raw).strip(" -•|@")
    return raw.upper() or "BET"
...
def _generate_pick_card(row: dict, forced_market_type: str | None = None) -> Path:
    ...
    group_x = text_left + max(0, (text_area_w - group_w) / 2) - 4
    ...
    content_left = board[0] + LEFT_GUTTER
    content_right = board[2] - RIGHT_GUTTER
    ...
    league_chip = (content_left, top_y, content_left + league_chip_w, top_y + chip_h)
    ...
    if primary_unit:
        ...
        _draw_text_centered(draw, unit_chip, primary_unit, unit_font, green, y_offset=1)
    ...
    if section:
        draw.text((content_left, section_y), section, font=_font(18, True), fill=(205, 211, 216))
        if play_count > 1:
            draw.text((content_left + _text_width(draw, section, _font(18, True)) + 14, section_y + 4), f"• {play_count} PLAYS", font=_font(14, True), fill=muted)
        draw.line((content_left, section_y + 28, content_right, section_y + 28), fill=(21, 30, 37), width=1)

    row_x1 = content_left
    row_x2 = content_right
    current_y = rows_top
